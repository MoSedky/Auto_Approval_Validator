...
def auto_approval_form():
    ...
    # else:
        if validate_after_auto_approval.partners_gov_ecr_saudi == 1:
            #   & validate_after_auto_approval.partners_cmp_saudi_more_than_one == 0:
            print('1.2. Attachment Needed, Send Request to Contract Reviewer then OTP Auto Approval')

        # elif (validate_after_auto_approval.partners_cmp_saudi_more_than_one == 0) \
        #         & (validate_after_auto_approval.partners_gov_ecr_saudi == 0):
        #
        #     print('1.3. No Attachments Needed, Send Request to Contract Reviewer then OTP Auto Approval ')
        # elif (validate_after_auto_approval.partners_cmp_saudi_more_than_one == 0) \
        #         & (validate_after_auto_approval.partners_gov_ecr_saudi == 1):
        #     print('1.4. Attachment Needed, Send Request to Contract Reviewer OTP Auto Approval ')

        if validate_after_auto_approval.partners_cmp_saudi_more_than_one == 1:
            print('1.1.1. Go to Validate Managers of Company has more than one Partners')
            ...
            if ((validate_after_auto_approval.partners_cmp_saudi_more_than_one == 1) &
                ...
                print('1.1.2. Go to Validate Partners')
                ...
                print('1.5. No documents Needed, Send Request to Contract Reviewer then OTP To Saudi or Resident')
...
def validate_partners_partner_cmp():
    ...
    if cmp_partner_partner_type_has_different_type == 0:
        print('1.1.2. No documents Needed. Send Request to Contract Reviewer then '
              'Send OTP to ECR Owner, Saudi Individuals or One Company Partner Owner')

    else:
        print('1.1.3. Please Attach Approval Documents and Send the Request to Contract Reviewer ')


Need_Auto_Approval = pre_condition()

if Need_Auto_Approval == 1:
    print('1.1. Yes Need Auto Approval, Lets validate Validation Cases for Reviewing')

    Need_Reviewing = validate_after_auto_approval()

    if Need_Reviewing == 1:
        print('1. Directly to Auto Approval')
    else:
        print('2. Reviewing needed by Contract Reviewer then Auto Approval')
    auto_approval_form()

else:
    print('3. No Auto Approval Needed, Request will assigned to Contract Reviewer then to Moj User')
